...
class HumanActor(BaseModel):
    ...
    surface_handle: str = Field(
        ..., description="Surface-specific handle (e.g. @mbychkowski)"
    )
...
class HumanInteractionEvent(BaseModel):
    ...
    issue_id: int | None = Field(
        default=None, description="GitHub Issue or Parent Issue ID"
    )
    comment_id: int | None = Field(
        default=None, description="GitHub Comment ID if applicable"
    )
    thread_ref: str = Field(
        ..., description="Canonical thread reference (e.g. github:issue:100)"
    )
    actor: HumanActor
    content: str = Field(..., description="Sanitized human message content")
    timestamp: str = Field(default_factory=lambda: datetime.now(UTC).isoformat())
    raw_payload: dict[str, Any] | None = Field(
        default=None, description="Original raw payload for audit"
    )
...
class GatewayResponse(BaseModel):
    status: str = Field(
        ...,
        description="Status of gateway processing (e.g. accepted, dropped, ignored_bot)",
    )
