...
from spec_engine.agents.council.tech_reviewer import (
    story_critic_agent,
    tech_reviewer_agent,
)
...
def loop_router(ctx: Context) -> Event:
    ...
    If less than 3 loops have completed, routes back to directly_responsible_agent ('loop_again').
    ...
    if rounds < 3:
        print(f"[Loop Router] Round {rounds} < 3: Routing back to Directly Responsible Agent for revision...")
...
root_workflow = Workflow(
    ...
    edges=[
        ...
        (directly_responsible_agent, tech_reviewer_agent),
        (tech_reviewer_agent, loop_router),
        (loop_router, {
